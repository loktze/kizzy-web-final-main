...
const PortalPhone: React.FC<PortalPhoneProps> = ({
  ...
  return (
    <div className={`h-full  flex justify-center items-center overflow-visible w-full `}>
      ...
      <div className={`${currentPage === 1 ? "scale-[1.3] translate-y-[22px]" : "scale-1 -translate-y-[150px]"} relative ${phoneClassName} origin-top duration-500 transition-all transform-gpu ease-in-out`}>
        {shouldShowPhone && (
          <img
            key="phone"
            ref={phoneRef}
